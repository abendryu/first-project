...
function adaptive_header (w,h) {
		...
		var headerClosed=$ ('.header-top-nav-menu');
	if(w<767) {
		if(!headerClosed.hasClass('done')){
			headerClosed.addClass('done').appendTo(headerMenu);
		}
	}else{
		if(headerClosed.hasClass('done')){
			headerClosed.removeClass('done') .prependTo($('.header-top-nav'));
		}
	}
}
